...
iptables -t nat -C POSTROUTING -s $VPN_LOCAL_SUBNET -o "$WAN_IFACE" -m policy --dir out --pol ipsec -j ACCEPT 2>/dev/null || \
iptables -t nat -A POSTROUTING -s $VPN_LOCAL_SUBNET -o "$WAN_IFACE" -m policy --dir out --pol ipsec -j ACCEPT
...
print_status() {
  echo "[*] Статус IKEv2/IPsec сервера:"
...
stop_ipsec() {
  echo "[*] Остановка IPsec..."
  systemctl stop "$IPSEC_SERVICE"
  echo "🛑 Остановлено."
}

restart_ipsec() {
  echo "[*] Перезапуск IPsec..."
